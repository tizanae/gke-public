...
module "gke_cluster" {
  ...
  name = "${var.cluster_prefix}-${var.environment}-k8s"
...
module "vpc_network" {
  ...
  network     =  var.network
  name_prefix = "${var.cluster_prefix}-network-${random_string.suffix.result}"
  project     = var.project
  region      = var.region
